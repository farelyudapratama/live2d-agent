...
static/vtuber.html          # halaman overlay OBS Browser Source (transparan)
...
- **📺 Overlay OBS**: tombol **Buka Overlay OBS** di panel VTuber membuka `static/vtuber.html?hud=1` — halaman transparan untuk **OBS Browser Source**. URL bersih `http://localhost:8310/vtuber.html` (tanpa `?hud=1`) = karakter Live2D bening + bubble bicara + banner donasi, tanpa UI. Overlay punya poller & balasan AI sendiri (gaya + jeda diatur di HUD-nya, tersimpan lokal); TTS & suara mengikuti config global + sheet model yang sama dengan app utama. Selama overlay terhubung (heartbeat `POST /api/vtuber/overlay` tiap 3 dtk), app utama **menahan balasan otomatisnya** (`overlay: true` di response `/api/vtuber/events`) supaya chat tidak dibalas dobel — feed di app utama tetap jalan.
