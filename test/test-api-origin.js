...
for (const [label, src] of [['js/app.js', appSrc], ['agent.js', agentSrc]]) {
...
// Both files are separate IIFEs, so each needs its own derivation.
for (const [label, src] of [['js/app.js', appSrc], ['agent.js', agentSrc]]) {
  ok(`${label}: declares its own API constant`,
    /const API\s*=\s*\(typeof location/.test(src));
}
...
section('server.js side of the contract');
const srvSrc = fs.readFileSync(path.join(ROOT, 'server.js'), 'utf8');
ok('server.js reads process.env.PORT', /Number\(process\.env\.PORT\)\s*\|\|\s*8310/.test(srvSrc));
ok('server.js default is still 8310 (no behaviour change for normal use)',
  /\|\|\s*8310/.test(srvSrc));
