/**
 * motion-taxonomy.js — classify Live2D .motion3.json clips into SEMANTIC VERBS.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 *
 * Runs in Node (server-side, reads files at import) AND in the browser
 * (fallback name-only classification). No dependencies.
 */
(function (root, factory) {
  const mod = factory();
  if (typeof module === 'object' && module.exports) module.exports = mod;
  else root.MotionTaxonomy = mod;
  ...
  // classification. Kept local so this module stays dependency-free and can
  // run server-side where app.js isn't loaded.
  const ROLE_PATTERNS = {
   ...
   *
   * @param {object} cdi3 parsed .cdi3.json (may be null)
   * @returns {{map:Object<string,string>, stats:{total:number, byId:number, byDisplay:number}}}
   */
  function buildRoleMap(cdi3) {
    const map = {};
    const stats = { total: 0, byId: 0, byDisplay: 0 };
  ...
  // endpoint.
  function decodeCurve(segments) {
    const pts = [];
    if (!Array.isArray(segments) || segments.length < 2) return pts;
    pts.push({ t: segments[0], v: segments[1] });
    let i = 2;
    let guard = 0;
    while (i < segments.length && guard++ < 100000) {
      const type = segments[i]; i++;
      if (type === 1) {
        if (i + 5 >= segments.length) break;
        pts.push({ t: segments[i + 4], v: segments[i + 5] });
        i += 6;
      } else if (type === 0 || type === 2 || type === 3) {
        if (i + 1 >= segments.length) break;
        pts.push({ t: segments[i], v: segments[i + 1] });
        i += 2;
      } else {
   ...
   * @param {object} motion3 parsed .motion3.json (may be null → name-only)
   * @param {string} name    group/file name, used only as a weak tie-breaker
   * @param {object} [roleMap] paramId -> role, from buildRoleMap(cdi3). Supply
   *        this for models whose parameter IDs are opaque (ParamEX10, Param92);
   *        without it those curves are invisible to the analyser.
   * @returns {{verb:string, valence:string, confidence:number, evidence:string, duration:number}}
   */
  function classifyClip(motion3, name, roleMap) {
    ...
    const F = (r) => byRole[r] || null;
    const amp = (r) => (byRole[r] ? byRole[r].amp : 0);
    // For every "does the clip HOLD this expression?" test we use the
    // TIME-weighted mean, not the keyframe mean — see curveFeatures().
    const mean = (r) => (byRole[r] ? byRole[r].tmean : 0);
    const rev = (r) => (byRole[r] ? byRole[r].reversals : 0);
   ...
   * @param {Array<{name:string, motion3?:object}>} clips
   * @param {object} [roleMap] paramId -> role from buildRoleMap(cdi3), applied
   *        to every clip. Strongly recommended: without it, models with opaque
   *        parameter IDs fall back to name guessing.
   * @returns {{byVerb:Object<string,string[]>, clips:Array, stats:Object}}
   */
  function buildTaxonomy(clips, roleMap) {
    const byVerb = {};
    const out = [];
    ...
    for (const c of clips || []) {
      ...
      const entry = { name: c.name, verb: r.verb, valence: r.valence, confidence: +r.confidence.toFixed(2), duration: r.duration, evidence: r.evidence };
      out.push(entry);
   ...
   *
   * @param {object} byVerb   verb -> [clip names]
   * @param {string} emotion  app emotion key ('senang', 'sedih', ...)
   * @param {function} rnd    optional RNG for deterministic tests
   */
  function pickClipForEmotion(byVerb, emotion, rnd) {
  ...
  function isCompatible(verb, emotion) {
    const allowed = EMOTION_VERBS[emotion] || EMOTION_VERBS.normal;
    return allowed.indexOf(verb) !== -1;
  }

  return {
    VERBS, VERB_VALENCE, EMOTION_VERBS,
    decodeCurve, curveFeatures, roleOf, nameHint,
    roleFromDisplayName, buildRoleMap,
    classifyClip, buildTaxonomy, pickClipForEmotion, isCompatible,
  };
});
